...
async function fetchHtml(url) {
  const res = await fetch(url, {
    headers: {
      'User-Agent':
        'Mozilla/5.0 (compatible; LetterboxdStatsBot/1.0; +https://github.com/your-username)'
    }
  });
  if (!res.ok) {
...
async function scrapeDiaryPage(username, page = 1) {
  ...
  $('.diary-entry-row').each((_, el) => {
    ...
    entries.push({
      ...
      Name: name,
      Year: yearText,
      LetterboxdURI: href ? `${LETTERBOXD_BASE}${href}` : '',
...
async function scrapeFullDiary(username, maxPages = 5) {
  ...
  while (page <= maxPages) {
    ...
    if (!entries.length && page === 1) break;
      ...
      chunk.map(async (movie) => {
        try {
          ...
          if (searchData.results && searchData.results.length > 0) {
            ...
            if (!detailsRes.ok) {
              throw new Error(`Details failed: ${detailsRes.statusText}`);
            }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!diaryEntries.length) {
      res.status(404).json({
        error:
          'No diary entries found. The profile may be private or the username invalid.'
      });
      return;
    }
